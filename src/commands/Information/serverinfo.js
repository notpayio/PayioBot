...
const disabled = '<:cross:1292960182657355847>'
const enabled = '<:tick:1292960123240845357>'
...
let emoji4 = `**Regular :** ${emojis.filter(emoji => !emoji.animated).size}/250\n**Animated :** ${emojis.filter(emoji => emoji.animated).size}/250\n**Total :** ${emojis.size}/500`;
  ...
  if(guild.premiumTier === 3 ) { emoji = `${emoji4}`
     ...
     value: `${features}`
            ...
            {
              name: '__Channels__',
              value: `**Total : ** ${channels.size}\n**Channels :** <:web:1292989719403036763> ${channels.filter(channel => channel.type === ChannelType.GuildText || channel.type === ChannelType.GuildForum || channel.type === ChannelType.GuildAnnouncement).size} | <:voice:1292960236210225173> ${channels.filter(channel => channel.type === ChannelType.GuildVoice || channel.type === ChannelType.GuildStageVoice).size} | <:folder:1293079957458915380> ${channels.filter(channel => channel.type === ChannelType.GuildCategory).size}\n**Rules Channel :**  ${guild.rulesChannelId ? `<#${guild.rulesChannelId}>`: `${disabled}`}`
            },
             {
              name: '__Emoji Info__',
              value: `${emoji}`
            },
            {
              ...
              value: `${booster[guild.premiumTier]} [<a:boost:1292960859177488424> ${guild.premiumSubscriptionCount || '0'} Boosts]`
            },
            {
    ...
    .setAuthor({name: `X ServerInfo Panel`,iconURL: client.user.displayAvatarURL()})
